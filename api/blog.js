...
import { slugify, readJsonBlob, proxiedImageUrl } from './_blog-utils.js';

// One function handles all blog operations, picked by HTTP method, so this
// only counts as a single Serverless Function against Vercel's limit:
//   GET    /api/blog?lang=es              -> list posts        (was blog-list.js)
//   POST   /api/blog        (auth)        -> create/update post (was blog-save.js)
//   DELETE /api/blog        (auth)        -> delete a post      (was blog-delete.js)
...
async function handleSave(request, response) {
  ...
  if (imageBase64) {
    try {
      ...
      const uploaded = await put(`blog-images/${langKey}-${slug}-${Date.now()}.${ext}`, buffer, {
        access: 'private', contentType: imageType || 'image/jpeg',
      });
  ...
  const post = {
    ...
    imagePath, author: session.username, publishedAt: incomingPublishedAt || now, updatedAt: now,
  };
